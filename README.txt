...
1) Create the appropriate mesh in ./meshes/  Not an exact science
   a) Modify mesh constants in naca_vX.geo (can modify from naca_orig.geo)
      1) Current streamlined process is by running find_hs.m to discover a mesh boundary layer with the appropriate connectivity
      2) Can input these constants into gmsh by rungmsh2msh() with optional parameters "-setnumber hLE XXX" etc
