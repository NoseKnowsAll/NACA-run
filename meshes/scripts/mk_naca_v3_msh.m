...
msh = rungmsh2msh(msh_dir+"naca_v3.geo", "-order "+string(p), "-setnumber hwing "+string(hwing), "-setnumber hLE "+string(hLE));
...
for refine = refinements
  ...
  if length(bnd_elems) > 0 && store_bl
    ...
    fid = fopen(msh_dir+"naca_v3_p"+string(p)+"_r"+string(refine)+"bl.mat", "w");
    fwritearray(fid, bnd_elems);
    fclose(fid);
  end
  h5fwritestruct(msh, msh_dir+"naca_v3_p"+string(p)+"_r"+string(refine)+".h5", dgfieldnames('msh'));
end
