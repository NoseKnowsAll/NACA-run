...
function naca_timing2d()
  ...
  presteps = 10;
  step0    = 0;    % set to 1 if you have precomputed solution
  writeint = 50;
  nstages  = 3;
  maxiter  = 300;
  linerror = 1e-4;
  ...
  if step0 == 0
    ...
    u = dgirktime(u,msh,data,phys,@dgnavierstokes, [dt/100,presteps,1], solver_parameters, 'gmres j');
    u = dgirktime(u,msh,data,phys,@dgnavierstokes, [dt/50, presteps,1], solver_parameters, 'gmres j');
    u = dgirktime(u,msh,data,phys,@dgnavierstokes, [dt/20, presteps,1], solver_parameters, 'gmres j');
    u = dgirktime(u,msh,data,phys,@dgnavierstokes, [dt/10, presteps,1], solver_parameters, 'gmres j');
  ...
  for i = step0+1:final_step
    fprintf("\n >>> Step %5d <<< \n", i);
    u = dgirktime(u,msh,data,phys,@dgnavierstokes, [dt,1,nstages], solver_parameters, 'gmres j');
    if mod(i, writeint) == 0
