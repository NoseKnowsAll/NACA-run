...
const int         step0    = 5000;   // set to 1 if you have precomputed solution
...
int main(int argc, char **argv) {
  ...
  if (step0 == 0) {
    ...
    fwritesolution(pre + "snaps/sol" + fill_int_to_string(1, 5, '0') + ".dat", u, msh);
  } else {
    // Don't compute initial steps. Load soln from file instead
    dgprintf(" >>> Checkpointing from %d <<<\n", step0);
    freadsolution(pre + "snaps/sol" + fill_int_to_string(step0, 5, '0') + ".dat", u, msh);
  ...
  for (int test = 1; test <= 5; test++) {
	...
	fwritearray(pre+"mass/benchmark_residual.mat", r);
	break;
      case res_eval:
	...
	fwritearray(pre+"mass/benchmark_matvec.mat", du);
	break;
      case mass_inv:
	dgmassinv(r, msh, d);
	break;
	...
	precond.mgpc(r);
	fwritearray(pre+"mass/benchmark_jacobi.mat", r);
	break;
      }
